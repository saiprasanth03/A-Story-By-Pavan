    ...
    <div className="min-h-screen bg-[#fafaf9] text-[#0f0f12] font-lato selection:bg-neutral-200 overflow-x-hidden relative">
        ...
        {showScrollTop && (
          <>
            ...
            <motion.button 
              ...
              className="fixed bottom-6 left-6 z-50 bg-white hover:bg-neutral-100 border border-black/15 text-[#0f0f12] w-11 h-11 rounded-full shadow-md flex items-center justify-center transition-all font-bold"
            >
              ↑
            </motion.button>
          </>
      ...
      <header className="fixed top-0 left-0 w-full z-50 px-6 py-4 flex justify-between items-center bg-[#050505]/90 backdrop-blur-md border-b border-white/10 shadow-lg">
        <button onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}>
          <img src={pageData?.logoUrl || "/images/logo.png"} alt={pageData?.heroSubheading || siteConfig.brand.name} className="h-11 w-auto object-contain" fetchpriority="high" />
        </button>
        <button 
          onClick={() => setIsModalOpen(true)}
      ...
      <section className="relative h-[90vh] sm:h-screen flex items-center justify-center overflow-hidden bg-black">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.4 }} className="flex flex-col items-center w-full">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-6 sm:gap-14 mt-2">
              <div className="flex flex-col items-center sm:items-start text-center sm:text-left">
                <span className="font-lato text-gray-300 uppercase tracking-widest text-xs font-bold mb-0.5">{pageData?.heroPriceText || 'Packages Start From Just'}</span>
                <span className="font-mirage text-white text-3xl sm:text-4xl font-bold">{pageData?.heroPriceAmount || '₹3,999/-'}</span>
              </div>
        ...
        <motion.div 
          ...
          className="absolute bottom-6 left-1/2 transform -translate-x-1/2 flex flex-col items-center z-20 pointer-events-none"
        >
          <span className="font-lato uppercase tracking-[0.3em] text-[10px] font-bold text-white mb-2">Scroll</span>
      ...
      {/* APPROACH SECTION */}
      <section className="py-16 px-6 lg:px-12 max-w-6xl mx-auto flex flex-col text-center">
        <h2 className="font-mirage font-bold text-4xl md:text-5xl uppercase tracking-[0.1em] mb-4 text-[#0f0f12]">
          {pageData?.approachHeading || "Our Approach"}
        </h2>
        <p className="font-lato font-light text-neutral-600 leading-relaxed tracking-wide text-sm md:text-base max-w-2xl mx-auto mb-12">
        ...
        {pageData?.approachSections && pageData.approachSections.length > 0 && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-left">
            {pageData.approachSections.map((item, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="bg-white border border-black/10 p-8 rounded-3xl shadow-sm hover:shadow-md transition-all">
                <div className="font-mirage text-3xl text-neutral-300 font-bold mb-3">0{i + 1}</div>
                <h3 className="font-mirage text-lg uppercase tracking-widest mb-2 text-[#0f0f12] font-bold">{item.heading}</h3>
                <p className="font-lato text-sm text-neutral-600 leading-relaxed">{item.description}</p>
              </motion.div>
            ))}
          </div>
        )}
            ...
            {pageData?.serviceCards && pageData.serviceCards.length > 0 ? pageData.serviceCards.map((card, idx) => (
              ...
              >
                ...
                <div className="p-7 relative z-20 bg-white">
                  ...
                  <h3 className="font-mirage text-2xl uppercase tracking-wider text-[#0f0f12] font-bold mb-2">{card.title}</h3>
                  <p className="font-lato text-neutral-600 font-light text-sm leading-relaxed">{card.description}</p>
                </div>
              </motion.div>
            )) : (
              <>
      ...
      <section className="py-16 px-6 max-w-5xl mx-auto">
        <div className="bg-white border border-black/15 p-8 md:p-12 rounded-3xl shadow-2xl relative overflow-hidden">
          <h3 className="font-mirage text-2xl sm:text-3xl uppercase tracking-widest mb-8 text-[#0f0f12] text-center font-bold">Book Your Shoot</h3>
      ...
      <section className="py-20 border-t border-black/10 overflow-hidden bg-white">
        <div className="px-6 lg:px-12 mb-10 flex flex-col items-center">
          <h2 className="font-mirage font-bold text-3xl md:text-4xl uppercase tracking-[0.2em] text-[#0f0f12]">{pageData?.portfolioImagesHeading || "Our Best Clicks"}</h2>
        </div>
                ...
                {(pageData?.features?.length > 0 ? pageData.features : [{ title: "40+ Premium Themes", description: "Amazing, hand-crafted setups for every mood." }, { title: "Certified Newborn Wraps", description: "Done by professionals ensuring 100% baby comfort." }, { title: "Cinematic Video & Editing", description: "Premium-grade videos and high-end photo retouching." }]).map((item, i) => (
                  <motion.div key={i} variants={fadeInUp} className="flex gap-4 items-start">
                    ...
                    <div>
                      ...
                      <p className="font-lato text-neutral-600 font-light text-sm">{item.description || item.desc}</p>
                    </div>
                  </motion.div>
                ))}
              </div>
      ...
      {pageData?.showVideoGallery !== false && (
        <section className="py-20 overflow-hidden bg-white">
          <div className="px-6 lg:px-12 mb-10 flex flex-col items-center">
            <h2 className="font-mirage font-bold text-3xl md:text-4xl uppercase tracking-[0.2em] text-[#0f0f12]">{pageData?.portfolioVideosHeading || "Memorable Client Stories"}</h2>
          </div>
          ...
          <div className="relative w-full px-6 lg:px-12 pb-8">
            ...
            >
              {[...portfolioVideos, ...portfolioVideos, ...portfolioVideos, ...portfolioVideos].map((vid, i) => {
                ...
                return (
                  <SwiperSlide key={i} className="!w-[300px] md:!w-[500px] aspect-video overflow-hidden rounded-2xl border border-black/10 relative cursor-pointer hover:border-black/30 transition-all shadow-md" onClick={() => setActiveVideo(videoId)}>
                    ...
                    <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                      <div className="w-14 h-14 rounded-full bg-white/90 border border-black/20 flex items-center justify-center shadow-lg backdrop-blur-xs">
                        <span className="text-[#0f0f12] text-lg ml-0.5 font-bold">▶</span>
                      </div>
                    </div>
                  </SwiperSlide>
                );
      ...
      <section className="relative py-28 text-center overflow-hidden border-t border-black/10 bg-black">
        ...
        >
          <h2 className="font-mirage text-4xl sm:text-6xl uppercase tracking-widest text-white font-bold mb-4 drop-shadow-md">{pageData?.parallaxFooter?.heading || 'Affordable Premium Baby Shoot'}</h2>
